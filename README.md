...
4. Open a browser and navigate to `http://localhost:4173` to view the application. You can also change the default port in the `package.json` file by adding `--port=YOUR_PORT` to the `scripts` object `dev` line or in `vite.config.ts` by adding the following object: `server: { port: YOUR_PORT }`.
